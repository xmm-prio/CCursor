...
// V4 pools redirected requests over a keep-alive agent; V3 added the routes
// channel subscription next to the routes.json watcher.
...
export const HTTP11_ROUTER_VERSION_MARKER = '__byokHttp11RouterV4';
export const HTTP11_ROUTER_SOURCE_MARKER = 'BYOK-HTTP11-ROUTER-V4';
...
export function isNodeHttp11RouterPatched(source, guardMarker) {
  ...
  return head.includes(`/* ${HTTP11_ROUTER_SOURCE_MARKER} */`)
    ...
    && head.includes('_directHttpOwner.Agent||_http.Agent')
    && head.includes('result.agent=_pool')
